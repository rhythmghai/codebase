...
class CrossEncoderReranker:
    ...
    def __init__(self, model_name: str = "cross-encoder/ms-marco-MiniLM-L-6-v2"):
        try:
            ...
            raise RuntimeError(
                f"Failed to load cross-encoder reranker model '{model_name}' -- "
                f"check that sentence-transformers is installed and the model "
                f"can be downloaded/is cached. Use get_reranker('lexical') as a "
                f"dependency-free fallback. Original error: {e}"
            ) from e
